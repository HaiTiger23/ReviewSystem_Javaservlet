...
/**
 * Lớp Data Access Object cho Product, cung cấp các phương thức để tương tác với cơ sở dữ liệu
 */
public class ProductDAO {
    ...
    public Map<String, Object> getProducts(int page, int limit, Integer categoryId, String search, String sort) {
        ...
        if (search != null && !search.trim().isEmpty()) {
            sqlBuilder.append(hasWhere ? " AND " : " WHERE ");
            sqlBuilder.append("MATCH(p.name, p.description) AGAINST(? IN NATURAL LANGUAGE MODE)");
            params.add(search);
            hasWhere = true;
        }
        ...
        if (hasWhere) {
            countSqlBuilder.append(" WHERE ").append(sqlBuilder.substring(sqlBuilder.indexOf(" WHERE ") + 7, sqlBuilder.indexOf(" ORDER BY ")));
        }
     ...
     * @param specifications Danh sách thông số kỹ thuật
     */
    private void addProductSpecifications(Connection conn, int productId, List<ProductSpecification> specifications) throws SQLException {
